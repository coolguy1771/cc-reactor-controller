...
Round 4 final GREEN: portable `turbine_dispatch.lua` passed all dispatch, regression, finite-limit, branch-failure, and invalid-gate assertions; `device_sampling.lua` and `reactor_control.lua` passed; `git diff --check` passed. `sim.lua` retains exactly 16 Task 6-only controller integration failures. Round 4 commits: `732481c`, `5cff376`, `69b45ba`, `714411b`, `490a559`. Coverage includes finite fallback/precedence boundaries, protected branch writes, constructor failures, exact probe behavior, and each invalid probe gate.
